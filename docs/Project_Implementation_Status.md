...
- CLI가 core 공통 모듈 재사용 + 설정 검증 적용: `uploader_cli.py`, `core/transform.py`, `core/config.py`, `core/upload.py`
...
- 리팩터링 문서의 검증/로그 표준화 일부 미반영(구조화 로그 포맷 없음, `scripts` 하위 검증 경로 부재): `uploader_gui_tk.py`, `uploader_cli.py`, `scripts`
